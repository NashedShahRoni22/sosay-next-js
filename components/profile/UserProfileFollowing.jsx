...
export default function UserProfileFollowing({ id, userInfo }) {
  const { accessToken } = useAppContext();
  const targetId = id || userInfo?.id;

  const { data, isLoading, error } = useQuery({
    queryKey: [`/friendship/following?user_id=${targetId}`, accessToken],
  ...
  if (error) {
    return (
      <p className="text-red-400 text-center mt-10">Failed to load following</p>
    );
  }
