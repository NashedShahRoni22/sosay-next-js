...
export default function PublicPost() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      // Return the page number with the data to prevent race conditions
      return { posts: postsRes, ads: adsRes, fetchedPage: page };
    },
  ...
  useEffect(() => {
    if (data?.posts?.data) {
      ...
      const fetchedPage = data.fetchedPage;

      if (fetchedPage === 1) {
        setAllPosts(newPosts);
        setFeedSections([{ page: fetchedPage, posts: newPosts, ads: newAds }]);
      } else {
        setAllPosts((prev) => {
        ...
        setFeedSections((prev) => {
          // Use fetchedPage instead of currentPage to check if it exists
          if (prev.some((s) => s.page === fetchedPage)) return prev;

          const existingPostIds = new Set(
            prev.flatMap((s) => s.posts.map((p) => p.id)),
          );
          ...
          const existingAdIds = new Set(
            prev.flatMap((s) => s.ads.map((a) => a.id)),
          );
          const uniqueAds = newAds.filter((a) => !existingAdIds.has(a.id));

          return [
            ...prev,
            { page: fetchedPage, posts: uniquePosts, ads: uniqueAds },
          ];
        });
      }
  ...
  }, [allPosts.length, hasNextPage, isFetching, currentPage]);
