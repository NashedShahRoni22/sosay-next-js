...
export default function MediaSwiper({ media, postId }) {
        ...
        <div className="mb-4 -mx-3 sm:mx-0 w-[calc(100%+1.5rem)] sm:w-full">
          {item.file_type === 1 ? (
            <button
              ...
              className="relative w-full max-h-[75vh] overflow-hidden sm:rounded-xl cursor-zoom-in block"
              style={
                item.aspectRatio
                  ? { aspectRatio: item.aspectRatio }
                  : { minHeight: "300px" }
  ...
  <div className="relative mb-4 -mx-3 sm:mx-0 w-[calc(100%+1.5rem)] sm:w-full overflow-hidden sm:rounded-xl group">
    <Swiper
        ...
        <SwiperSlide key={idx}>
          {item.file_type === 1 ? (
            <button
              type="button"
              onClick={() => openLightbox(item.file_name)}
              className="relative w-full h-[280px] xs:h-[320px] sm:h-[380px] md:h-[450px] lg:h-[500px] overflow-hidden flex items-center justify-center cursor-zoom-in"
            >
              <Image
                src={item.file_name}
